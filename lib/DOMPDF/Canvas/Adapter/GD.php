...
class GD implements Canvas
{
     ...
     */
    public function __construct($size, $orientation = "portrait", DOMPDF $dompdf, $aa_factor = 1.0, $bg_color = array(1, 1, 1, 0))
    {
        if (!is_array($size)) {
    ...
    public function stream($filename, $options = null)
    {
        ...
        if ($this->_aa_factor != 1) {
            ...
            imagecopyresampled($dst, $this->_img, 0, 0, 0, 0, $dst_w, $dst_h, $this->_width, $this->_height);
        } else {
            $dst = $this->_img;
        }
