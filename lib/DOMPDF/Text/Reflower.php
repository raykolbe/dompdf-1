...
class Reflower extends FrameReflower 
{
    ...
    public function __construct(TextDecorator $frame)
    {
...
//       $text = ltrim($text, " \n\r\t");
    ...
    protected function _layout_line()
    {
        ...
        switch (strtolower($style->text_transform)) {
            default: break;
            case "capitalize": $text = mb_convert_case($text, MB_CASE_TITLE);
                break;
            case "uppercase": $text = mb_convert_case($text, MB_CASE_UPPER);
                break;
            case "lowercase": $text = mb_convert_case($text, MB_CASE_LOWER);
                break;
        }
